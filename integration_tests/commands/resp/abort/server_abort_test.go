...
func init() {
	config.GlobalDiceDBConfig.Port = testServerOptions.Port
	log.Print("Setting port to ", config.GlobalDiceDBConfig.Port)
}
...
func TestAbortCommand(t *testing.T) {
	...
	t.Run("ServerIsRunning", func(t *testing.T) {
		conn, err := net.Dial("tcp", fmt.Sprintf("127.0.0.1:%d", config.GlobalDiceDBConfig.Port))
		if err != nil {
	...
	t.Run("AbortCommandShutdown", func(t *testing.T) {
		...
		_, err = net.Dial("tcp", fmt.Sprintf("127.0.0.1:%d", config.GlobalDiceDBConfig.Port))
		if err == nil {
	...
	t.Run("PortIsReleased", func(t *testing.T) {
		...
		listener, err := net.Listen("tcp", fmt.Sprintf("127.0.0.1:%d", config.GlobalDiceDBConfig.Port))
		if err != nil {
...
func TestServerRestartAfterAbort(t *testing.T) {
	...
	conn2, err := net.Dial("tcp", fmt.Sprintf("127.0.0.1:%d", config.GlobalDiceDBConfig.Port))
	if err != nil {
		t.Fatalf("Server should be running after restart: %v", err)
	}
