...
func TestCMSInitByProb(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			DeleteKey(t, conn, exec, "cms_key1")
			for i, cmd := range tc.commands {
				result, err := exec.FireCommandAndReadResponse(conn, cmd)
				assert.Nil(t, err)
				assert.Equal(t, tc.expected[i], result)
...
func TestCMSInfo(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			DeleteKey(t, conn, exec, "cms_key2")
			for i, cmd := range tc.commands {
				result, err := exec.FireCommandAndReadResponse(conn, cmd)
				assert.Nil(t, err)
				assert.Equal(t, tc.expected[i], result)
...
func TestCMSIncrBy(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			DeleteKey(t, conn, exec, "cms_key3")
			for i, cmd := range tc.commands {
				result, err := exec.FireCommandAndReadResponse(conn, cmd)
				assert.Nil(t, err)
				assert.Equal(t, tc.expected[i], result)
...
func TestCMSQuery(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			DeleteKey(t, conn, exec, "cms_key4")
			for i, cmd := range tc.commands {
				result, err := exec.FireCommandAndReadResponse(conn, cmd)
				assert.Nil(t, err)
				assert.Equal(t, tc.expected[i], result)
...
func TestCMSMerge(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expected: []interface{}{
				...
				[]interface{}{float64(20), float64(20)},
				"OK",
				[]interface{}{float64(50), float64(60)},
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			DeleteKey(t, conn, exec, "cms_key5")
			DeleteKey(t, conn, exec, "test")
			DeleteKey(t, conn, exec, "test1")
			for i, cmd := range tc.commands {
				result, err := exec.FireCommandAndReadResponse(conn, cmd)
				assert.Nil(t, err)
				assert.Equal(t, tc.expected[i], result)
