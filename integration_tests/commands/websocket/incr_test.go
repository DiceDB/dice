	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, key := range keys {
				exec.FireCommand(conn, fmt.Sprintf("DEL %s", key))
			}

			for _, cmd := range tc.commands {
				switch cmd.op {
				case "s":
					exec.FireCommand(conn, fmt.Sprintf("SET %s %v", cmd.key, cmd.val))
				case "se":
					exec.FireCommand(conn, fmt.Sprintf("SET %s %v EX 1", cmd.key, cmd.val))
				case "i":
					result := exec.FireCommand(conn, fmt.Sprintf("INCR %s", cmd.key))
				...
				case "g":
					result := exec.FireCommand(conn, fmt.Sprintf("GET %s", cmd.key))
...
func TestINCRBY(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			defer exec.FireCommand(conn, "DEL key unsetKey stringkey")

			for _, cmd := range tc.setCommands {
				exec.FireCommand(conn, fmt.Sprintf("SET %s %d", cmd.key, cmd.val))
			}
			...
			for _, cmd := range tc.incrByCommands {
				...
				case int64:
					result = exec.FireCommand(conn, fmt.Sprintf("INCRBY %s %d", cmd.key, v))
				case string:
					result = exec.FireCommand(conn, fmt.Sprintf("INCRBY %s %s", cmd.key, v))
				}
				switch v := result.(type) {
				case string:
					assert.Equal(t, cmd.expectedErr, v)
