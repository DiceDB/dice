...
import (
	"context"
	"fmt"
	"github.com/dicedb/dice/integration_tests/command_tests"
	"net"
	"sync"
	"testing"
	"time"
...
var testServerOptions = command_tests.TestServerOptions{
	Port: 8740,
}
...
func TestAbortCommand(t *testing.T) {
	...
	command_tests.RunTestServer(ctx, &wg, testServerOptions)
	...
	t.Run("AbortCommandShutdown", func(t *testing.T) {
		...
		// Send ABORT command
		result := command_tests.FireCommand(conn, "ABORT")
		if result != "OK" {
...
func TestServerRestartAfterAbort(t *testing.T) {
	...
	command_tests.RunTestServer(ctx2, &wg2, testServerOptions)
	...
	// Clean up
	result = command_tests.FireCommand(conn2, "ABORT")
	if result != "OK" {
		t.Fatalf("Unexpected response to ABORT command: %v", result)
	}
	conn2.Close()
