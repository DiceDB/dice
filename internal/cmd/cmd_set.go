...
func parseParams(args []string) (params map[types.Param]string, nonParams []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		case types.XX, types.NX, types.KEEPTTL, types.LT, types.GT, types.CH, types.INCR:
			params[arg] = "true"
		case types.M, types.KM, types.MI, types.FT,
			types.WITHCOORD, types.WITHDIST, types.WITHHASH,
			types.FROMLONLAT, types.FROMMEMBER,
			types.BYBOX, types.BYRADIUS,
			types.ASC, types.DESC,
			types.COUNT, types.ANY:
			params[arg] = "true"
		default:
