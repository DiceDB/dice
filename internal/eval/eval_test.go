...
func testEvalPTTL(t *testing.T, store *dstore.Store) {
	tests := map[string]evalTestCase{
		...
		"key exists not expired": {
			...
			newValidator: func(output interface{}) {
				assert.Assert(t, output != nil)
				assert.Assert(t, output != clientio.IntegerNegativeOne)
				assert.Assert(t, output != clientio.IntegerNegativeTwo)
			},
		},
		"key exists but expired": {
			setup: func() {
	...
	tests := map[string]evalTestCase{
		...
		"set and get": {
			...
			newValidator: func(output interface{}) {
				...
				assert.Assert(t, ok, "TTL result should be an uint64")
				assert.Assert(t, ttl > 0 && ttl <= 5, "TTL should be between 0 and 5 seconds")
				assert.NilError(t, ttlResponse.Error, "TTL command should not return an error")
		...
		"update existing key": {
			...
			newValidator: func(output interface{}) {
				...
				ttl, ok := ttlResponse.Result.(uint64)
				assert.Assert(t, ok, "TTL result should be an uint64")
				assert.Assert(t, ttl > 0 && ttl <= 10, "TTL should be between 0 and 10 seconds")
				assert.NilError(t, ttlResponse.Error, "TTL command should not return an error")
			},
		},
	}
