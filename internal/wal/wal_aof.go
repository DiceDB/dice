...
func NewAOFWAL(directory string) (*AOF, error) {
	...
	return &AOF{
		logDir:                 directory,
		walMode:                config.GlobalDiceDBConfig.WALMode,
		bufferSyncTicker:       time.NewTicker(time.Duration(config.GlobalDiceDBConfig.WALBufferSyncIntervalMillis) * time.Millisecond),
		segmentRotationTicker:  time.NewTicker(time.Duration(config.GlobalDiceDBConfig.WALMaxSegmentRotationTimeSec) * time.Second),
		segmentRetentionTicker: time.NewTicker(time.Duration(config.GlobalDiceDBConfig.WALMaxSegmentRetentionDurationSec) * time.Second),
		writeMode:              config.GlobalDiceDBConfig.WALWriteMode,
		maxSegmentSize:         config.GlobalDiceDBConfig.WALMaxSegmentSizeMB * 1024 * 1024,
		maxSegmentCount:        config.GlobalDiceDBConfig.WALMaxSegmentCount,
		bufferSize:             config.GlobalDiceDBConfig.WALBufferSizeMB * 1024 * 1024,
		retentionMode:          config.GlobalDiceDBConfig.WALRetentionMode,
		recoveryMode:           config.GlobalDiceDBConfig.WALRecoveryMode,
		rotationMode:           config.GlobalDiceDBConfig.WALRotationMode,
		ctx:                    ctx,
		cancel:                 cancel,
	}, nil
}
