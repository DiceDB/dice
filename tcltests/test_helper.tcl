...
source tcltests/support/redis.tcl
source tcltests/support/aofmanifest.tcl
source tcltests/support/server.tcl
source tcltests/support/cluster_util.tcl
source tcltests/support/tmpfile.tcl
source tcltests/support/test.tcl
source tcltests/support/util.tcl
...
foreach test_dir $test_dirs {
    set files [glob -nocomplain $dir/tcltests/$test_dir/*.tcl]
...
set ::host 127.0.0.1
set ::port 7379; # port for external server
...
proc execute_test_file __testname {
    set path "tcltests/$__testname.tcl"
    set ::curfile $path
    source $path
...
proc cleanup {} {
    ...
    flush stdout
    catch {exec rm -rf {*}[glob tcltests/tmp/redis.conf.*]}
    catch {exec rm -rf {*}[glob tcltests/tmp/server.*]}
    if {!$::quiet} {puts "OK"}
}
