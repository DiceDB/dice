	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, key := range keys {
				client.FireString(fmt.Sprintf("DEL %s", key))
			}

			for _, cmd := range tc.commands {
				switch cmd.op {
				case "s":
					client.FireString(fmt.Sprintf("SET %s %v", cmd.key, cmd.val))
				case "se":
					client.FireString(fmt.Sprintf("SET %s %v EX 1", cmd.key, cmd.val))
				case "i":
					result := client.FireString(fmt.Sprintf("INCR %s", cmd.key))
					if result.GetVStr() != "" {
						assert.Equal(t, cmd.expectedErr, result.GetVStr())
					} else {
						assert.Equal(t, cmd.val, result.GetVInt())
					}
				case "g":
...
func TestINCRBY(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			defer client.FireString("DEL key unsetKey stringkey")
			...
			for _, cmd := range tc.incrByCommands {
				...
				case int64:
					result = client.FireString(fmt.Sprintf("INCRBY %s %d", cmd.key, v))
				case string:
					result = client.FireString(fmt.Sprintf("INCRBY %s %s", cmd.key, v))
				}
